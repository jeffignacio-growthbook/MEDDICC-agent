...
- **Budget:** on the live 2026-09-25 inputs the downside quarter-health view is 19,111 chars compact against `SYNTH_PAYLOAD_CHARS` 20,000 (base: 19,050 indented, fits without compacting). `tests/test_quarter_health_qtd_and_wording.py` fails if a tail cut starts dropping rows; the next thing added to the composed view needs a cut elsewhere first.
