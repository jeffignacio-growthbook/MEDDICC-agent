...
def build_semantic_context(config: Optional[Dict] = None) -> str:
    ...
    if targets_path.exists():
        ...
        lines.append("  ✓ Conversion rate comes from metrics registry: 39.0% blended (trailing 3Q average)")
        lines.append("  ✓ Segment rates: Enterprise 27.8%, Mid-Market 43.3%, SMB 51.2%")
        ...
        lines.append("  Formula: required_pipeline = target ÷ segment_conversion_rate")
        lines.append("  Example: At 39.0% blended, $2.1M target needs $5.4M qualified pipeline")
        lines.append("  IMPORTANT: Use segment-specific rates when available (Mid-Market 43.3% vs Enterprise 27.8%)")
        lines.append("  DO NOT use coverage multiples (2.5x, 3x, etc.) - use verified conversion rates")
        lines.append("")
