#!/usr/bin/env python3
"""
Regression test: query_stale_deals must match frozen baseline.

Compares current handler output against tests/fixtures/query_stale_deals_baseline.json
for all test cases. Uses 0.01 float tolerance for numeric comparisons.
"""
import asyncio
import json
import sys
...
async def test_regression():
    """Run all test cases and compare against frozen baseline."""
    # Load frozen baseline
    baseline_path = repo_root / "tests/fixtures/query_stale_deals_baseline.json"
    ...
    for i, tc in enumerate(baseline['test_cases'], 1):
        ...
        # Run current handler
        try:
            current_result = await query_stale_deals(params, sb)
        ...
        critical_fields = ['stale_count', 'past_close_date_count', 'total_stale_pipeline',
                          'stale_threshold_days', 'stale_deals']
        ...
        if matches:
            stale_count = current_result.get('stale_count')
            past_close = current_result.get('past_close_date_count')
            deals_len = len(current_result.get('stale_deals', []))
            print(f"   ✅ MATCH (stale={stale_count}, past_close={past_close}, deals={deals_len})")
        else:
            all_passed = False
