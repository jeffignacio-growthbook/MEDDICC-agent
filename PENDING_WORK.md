...
- **Budget:** the two quarter-health entry points have their own synthesis budget, `router.SYNTH_PAYLOAD_CHARS_BY_HANDLER` 26,000 (every other handler: `SYNTH_PAYLOAD_CHARS` 20,000). After the 2026-09-25 reframe (five primitives, pace, coverage) on the live inputs: base 25,472 chars indented (fits, sent indented), downside 24,528 compact. About 1,500 chars of headroom; the survival tests fail if the cut ever fires. The dynamic loop sends structured-handler results whole, so the limit binds on the classifier path only.
