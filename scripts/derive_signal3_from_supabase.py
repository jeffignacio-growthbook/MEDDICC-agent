#!/usr/bin/env python3
"""
Signal 3 Derivation from Supabase property_history table

Applies:
1. Bulk event exclusion (10 cleanup months from business_events_registry.yaml)
...
def main():
    ...
    print("=" * 80)
    print("SIGNAL 3 DERIVATION FROM SUPABASE")
    ...
    for deal in closed_deals:
        ...
        # EXCLUSION 1: Filter out bulk cleanup months
        activities = []
        for change in nlu_history:
            changed_at = change['changed_at']

            if is_bulk_cleanup_month(changed_at):
                stats['activities_excluded_bulk'] += 1
                continue
    ...
    print(f"  Activities excluded (bulk): {stats['activities_excluded_bulk']}")
    ...
    output_file = Path(__file__).parent.parent / 'signal3_final_results.json'
