...
HANDLER_DESCRIPTIONS = {
    ...
    "query_loss_concentration": (
        ...
        "team average, never a bare count. Leads with the QUALIFIED loss "
        "rate (Sales deals that reached Discovery or later, not closed as "
        "Disqualified) and gives the all-closed rate beside it. Use for: "
        "'what's our loss rate', 'which rep has the "
...
{{
  ...
  "params": {{
    ...
    "pipeline_id": "<for query_pipeline_movement: specific HubSpot pipeline ID string when question asks about a specific pipeline by ID. Null for default scope (excludes renewals)>",
    "pipeline_filter": "new_business|renewal|null — for query_pipeline and query_pipeline_movement: set to 'new_business' when question explicitly asks for new business or incremental pipeline (e.g., 'new business pipeline', 'current new business pipeline', 'incremental pipeline'); set to 'renewal' for renewal-specific questions; null for all pipelines (default)",
    ...
    "component": "MEDDICC component name for filtering/analysis (champion, economic_buyer, metrics, decision_criteria, decision_process, pain, competition). Null for all components (default). Handlers: query_rubric, submit_score_correction, query_coaching_priorities.",
...
TOOLS YOU CAN CALL:
  ...
  query_stale_deals(owner_email, stage, stale_days, time_window)
    ...
    **USE THIS when the question asks about**:
    - STALE DEALS, DEALS HAVEN'T MOVED, STUCK DEALS, INACTIVE DEALS
    - Deals with no activity for extended period
    - Deals past their close date
    - "Which deals are stale", "show me stale deals", "deals stuck in [stage]"
    Params:
    - owner_email: filter to specific rep (optional, accepts email or name)
    - stage: filter to specific stage name (optional, e.g. "Discovery", "Technical Evaluation")
    - stale_days: number of days threshold for staleness (optional, defaults to 21)
