"""
Context Builder for MEDDICC Agent (Kimi K3 Version)

Builds cumulative MEDDICC state from historical call summaries using Kimi K3.
~70% cost reduction vs Claude Haiku.
"""
import os
import json
...
def build_cumulative_meddicc(call_summaries: List[str], company: str) -> dict:
    """
    Build cumulative MEDDICC state from all historical call summaries using Kimi K3.
...
6. Output ONLY valid JSON with no additional text or markdown"""
...
Output ONLY valid JSON with no additional text."""
    ...
    try:
        ...
        meddicc_state["_cost"] = result["cost"]
        meddicc_state["_model"] = "kimi_k3"

        return meddicc_state

    except json.JSONDecodeError as e:
        print(f"Failed to parse Kimi JSON response: {e}")
...
- Provide security questionnaire responses"""
    ...
    print(f"Cost: ${result.get('_cost', 0):.6f} (Kimi K3)")
