...
- **Budget:** the two quarter-health entry points have their own synthesis budget, `router.SYNTH_PAYLOAD_CHARS_BY_HANDLER` 26,000 (every other handler: `SYNTH_PAYLOAD_CHARS` 20,000). On the live 2026-09-25 inputs with the per-rep "still live" lines: downside 20,706 chars compact, base 17,745 compact (20,657 indented, so it goes compact too). `tests/test_quarter_health_rep_context.py` and the other survival tests fail if the cut ever fires; the dynamic loop sends structured-handler results whole, so the limit binds on the classifier path only.
