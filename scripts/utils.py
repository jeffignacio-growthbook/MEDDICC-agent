...
def build_semantic_context(config: Optional[Dict] = None) -> str:
    ...
    if targets_path.exists():
        ...
        lines.append("  ✓ Conversion rate comes from metrics registry: 12.7% blended (trailing 3Q average)")
        lines.append("  ✗ Segment rates pending verification - use blended 12.7% for all segments")
        ...
        lines.append("  Formula: required_pipeline = target ÷ conversion_rate")
        lines.append("  Example: At 12.7% blended, $2.1M target needs $16.5M qualified pipeline")
        lines.append("  IMPORTANT: Do not use unverified segment rates - blended rate only until verified")
