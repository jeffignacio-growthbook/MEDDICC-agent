#!/usr/bin/env python3
"""
Extract structured feature gaps from resolved calls in the calls table.
...
def main():
    ...
    # - Intent suggests feature gap extraction is appropriate
    candidate_calls = []
    for call in all_calls:
        ...
        if intent and intent in ENRICHMENT_PROFILE:
            ...
            if profile.get('extract_feature_gaps'):
                candidate_calls.append(call)

    print(f"Found {len(candidate_calls)} unscanned non-internal calls eligible for feature gap extraction")
    ...
    for call_data in to_process:
        ...
        # but double-check profile allows feature gap extraction
        profile = ENRICHMENT_PROFILE.get(call_intent, {})
        if not profile.get("extract_feature_gaps"):
            print(f"  ↷ {company}: "
                  f"{call_intent} call — "
        ...
        try:
            ...
            _stamp(sb, call_id, JOB_NAME, company or 'unknown', len(gaps))
            scanned += 1
            companies.add(company or 'unknown')
            if deal_id:
                deal_id_resolved += 1
            print(f"  ✓ {company}: {len(gaps)} gap(s)")
