...
### 🔵 NEXT (own scoping pass, do not build with Tier A): Tier B root-cause for rate/ratio metrics (logged 2026-09-25)

- **Tier A shipped:** `scripts/analytics/metric_reconciliation.py` decomposes the change in a *sum-over-deals* metric (pipeline value, deal count, forecast total) between two captures into one bucket per deal — new / rescoped in / value edited / exited (won / lost / still-open-rescoped / unknown) / unchanged / unexplained residual — with exit outcome read from the deal's real current status, never inferred from absence. It closes exactly or states the residual. Validated against the real +$280,000 FY2027 Q3 waterfall change (Cursor +400K rescoped in, Virgin Media O2 −100K exited-still-open, Reliance −20K value edited): `tests/test_metric_reconciliation.py` + fixture `tests/fixtures/metric_reconciliation_waterfall_2026_09_24.json`. Standalone and read-only; no live handler yet.
- **Tier B stage 1 shipped:** `scripts/analytics/rate_mix_decomposition.py` decomposes the change in a weighted-average rate (loss rate, win rate) between two populations into rate effect (Σ w_old·Δr), mix effect (Σ r_old·Δw) and interaction (Σ Δw·Δr), which sum to the observed change exactly (arithmetic identity — no residual bucket). The segment dimension is a parameter (rep, company segment, forecast tier, depth tier). Validated on real FY2027 Q3 data: Christian's 100% loss rate vs the team's 77.8% is a **rate effect** (his past-Discovery deals convert worse), while Enterprise's 100% is a **pure mix effect** (its pool is 100% Discovery-only) — opposite conclusions, the Simpson's-paradox discrimination the stage exists to make. `tests/test_rate_mix_decomposition.py` + fixture `tests/fixtures/qualified_loss_fy2027_q3_2026_09_25.json`; 6/6 planted bugs caught. Standalone, read-only, no handler.
- **Tier B stage 2 deferred (not built):** dimensional root-cause ranking (Adtributor-style explanatory-power × surprise) to find which slice(s) carry a rate anomaly when the rate effect dominates. Build only after stage 1's validation is accepted; validate it surfaces Enterprise/Christian without being pointed at them.
- **Tier B stage 3 deferred (not built):** Shapley/counterfactual multi-factor attribution for overlapping factors (e.g. Christian ∩ Enterprise). Build only if a real investigation shows more than one contributing factor via stage 2 — do not build the general machinery speculatively.
...
- **What:** `stage_of_loss.by_bucket` and `administrative_stage_share` bucket losses by `deals.highest_stage_order_reached`. That column is a high-water mark over config orders, and the terminal stages sit above the funnel: Closed Lost is 7, Review 8, Disqualified 9. So every Sales loss reads 7 or higher, and the buckets say nothing about how deep the deal got. Live FY2027 Q3: `closed_lost` 85, `proposal` 24 (15 losses at order 8 plus the 9 renewal losses at renewal order 5).
- **Fix when picked up:** take depth from `deals_snapshot` (the deepest Discovery-to-Awaiting-Signature stage seen before close), which the qualified loss rate already fetches, and over qualified losses only. Tests first on the live capture `tests/fixtures/loss_concentration_fy2027_q3_2026_09_25.json`.

### 🟢 LOW, watch: SAO vs stage history disagree on older deals; composed downside view is near the synthesis budget (logged 2026-09-25)

- **SAO:** the qualified loss rate uses stage history (seen at Discovery or later before close). The earlier audit's 20.5% win / 79.5% loss used the HubSpot `sao` flag (8 of 39). For FY2027 Q3 the two definitions agree on every deal that has both; the whole difference is 3 closed-lost deals with `sao` true and no snapshot history, which the headline reports ("if all 3 had qualified: 79.5%"). Across Sales deals closed since 2026-02-01 they disagree on 46: 36 with `sao` false were seen at Discovery or later, 10 with `sao` true never were. Worth knowing before anyone reports a win rate "by SAO".
- **Budget:** the two quarter-health entry points have their own synthesis budget, `router.SYNTH_PAYLOAD_CHARS_BY_HANDLER` 26,000 (every other handler: `SYNTH_PAYLOAD_CHARS` 20,000). After the per-rep loss-depth + win-pace lines (rep_scorecard, 2026-09-25) on live inputs: base 25,789 chars indented (fits, sent indented), downside 24,867 compact. ~1,100 chars of headroom in the downside view; the survival tests fail if the cut fires. The dynamic loop sends structured-handler results whole, so the limit binds on the classifier path only.

### 🔵 BLOCKED (on history, not code): quarter-over-quarter trend in the quarter-health read (logged 2026-09-25)

- **What:** "are we better or worse placed than at this point last quarter?" — coverage of the remaining gap, pace, qualified loss rate and the forecast risk share at the same week of prior quarters, side by side.
- **Why it's blocked:** the figures only became comparable on 2026-09-25. The qualified loss rate definition, the current-stage coverage key and the renewal exclusion from risk all changed that day, so no earlier quarter has these numbers under the same definitions, and reconstructing them from snapshots would mix today's rules with history that wasn't recorded for them. Same gate as the qualification-stage and commit-reliability items: wait for 2-3 complete quarters of comparable history.
- **To unblock:** record the composed figures (QTD won, target, weighted coverage and remaining gap, pace, qualified and all-closed loss rates, forecast risk shares) once a week under fixed definitions, starting now; build the comparison once 2-3 complete quarters exist. Until then the verdict compares nothing across quarters.

### 🟠 MEDIUM: specify and verify the forecast-trust historical cohort before it returns to any verdict (logged 2026-09-25)

- **What:** `assess_forecast_trust`'s `historical` block, the same-week win rate of prior quarters' COMMIT + MOST_LIKELY deals (`forecast_analyses.query_commit_ml_calibration_by_week`; 31.9%, n = 91 at week 8 on 2026-09-25), and the plain-language sentence built on it.
- **Status:** left out of the quarter-health verdict (`api/quarter_health.VERDICT_EXCLUDED`): its figure, note and calibration evidence never reach synthesis from the composer. It led to three wrong conclusions in a row on 2026-09-25. It is still returned by `query_forecast_trust` for direct questions.
- **Before it comes back, pin down and test on a live capture:** which complete quarters are pooled and why; which snapshot defines "tagged at week N"; how slipped, renewal and zero-ARR deals are counted; whether the 91 are deals or deal-weeks; and that won / (won + lost) matches a hand count from `deals_snapshot`. Then decide whether it belongs in the verdict at all, next to the stage-weighted coverage.

### ✅ FIXED 2026-09-25: `query_pipeline`'s standalone note put "COVERAGE: 3.38x" into the quarter-health view

- The composed view lifted `query_pipeline._synthesis_note` verbatim; written for a standalone pipeline answer, it states the unweighted coverage ratio ("COVERAGE: 3.38x") and stage-breakdown/top-deal rules, in an answer whose only coverage figure is the weighted one. It is now in `api/quarter_health.VERDICT_EXCLUDED` (dropped before lifting); its definitional rule (the total is current state, not this quarter's) reaches the model through `business_definition_note` and the composer note. `tests/test_quarter_health_reframe.py` checks no 3.38x / ratio value / "COVERAGE:" text reaches any model input. `query_pipeline` answers on their own are unchanged.
