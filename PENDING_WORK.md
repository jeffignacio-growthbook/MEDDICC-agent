...
#### 12. Dimension-Verification Warning Observed to Dead-End Without Forcing a Retry — NOT YET ROOT-CAUSED, UNRELATED TO #9/#10/#11
...
`⚠️ Verification failed: Question asked about ... Required query: ...`
warning appeared and the loop stopped there instead of forcing the
retry this message is supposed to trigger. Not reproduced or
diagnosed by Claude in this session — logged from what was observed
live, to be investigated properly rather than guessed at now.

**What this message is:** `api/dimension_verification.py`'s
`format_verification_error()` — the human-readable message built when
`verify_dimension_coverage()` finds the question mentioned a dimension
value that was never actually filtered for. This is a real, existing
primitive (`PRIMITIVE_CHECKLIST.md` already tracks it), called from
five sites in `api/router.py`.

**A partial head start, not a diagnosis:** reading all five call
sites tonight (without reproducing the live failure) found four are
correctly wired: two mid-loop paths (~3641 "prose path", ~3998 main
MANDATORY GATE) explicitly force a retry (`messages.append(...);
continue`) on a failed verification; one (~2965, inside the
budget-exhausted diagnostic-message builder) only branches on the
*success* case, which is correct there since there's no budget left to
retry with; and one (~4217) isn't a retry-forcing gate at all — it
checks whether a *previous* forced retry just succeeded, to short-
circuit into synthesis. The fifth site (~3374, explicitly commented
"MANDATORY GATE: Dimension coverage verification (finalization path)")
is the one place verification failure does NOT force a retry — its own
comment says why: "Cannot retry (budget exhausted). Returning
diagnostic error." — a deliberate, documented design choice for the
last-resort finalize step, not obviously a bug.

**The open question, unresolved:** did the observed dead end hit that
one legitimate no-retry finalization path (in which case this may be
expected behavior working as designed, not a bug — though possibly one
worth reconsidering if it happens more than the "genuinely pathological
question" case it was built for) — or is there a sixth, different,
incompletely-wired check this quick read didn't find? The two
described in the original ask ("the known dimension-verification
primitive missing a retry wire-up, or a different, incomplete check
entirely") are both still live possibilities. Needs the actual failing
question/transcript to reproduce and trace which of the five sites (or
some other code path) actually fired, before concluding either way.

**Work:** Reproduce with the specific ARR question(s) that triggered
it; capture the log line naming which primitive/site fired (each site
above logs distinctly — `[DIMENSION_VERIFY]` variants); trace whether
it's the finalization path (expected, possibly still worth revisiting)
or a genuinely unwired site; fix or reclassify accordingly.

**Complexity:** Unknown until reproduced — could be "this is working
as designed, just confusing" (low) or a real missing-retry bug at a
site this read didn't catch (medium).

**Documentation:** none yet — first surfaced live tonight, not yet
reproduced or root-caused in this session.
