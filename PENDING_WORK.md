...
- **Tier B, not built:** rate / ratio metrics (win rate, coverage multiple, loss rate) don't decompose by summing deal contributions — a ratio moves through numerator, denominator, and mix shifts at once. This needs genuine hypothesis testing (which of {numerator, denominator, segment mix} moved, and is the move outside noise), not a per-deal waterfall. Scope it on its own before writing anything; do not fold it into the Tier A module.
