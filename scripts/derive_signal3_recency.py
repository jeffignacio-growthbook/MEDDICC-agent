#!/usr/bin/env python3
"""
Signal 3 Derivation - CORRECTED: Recency, not Frequency

For each deal, compute ONE number:
- Gap between LAST activity (notes_last_updated change) and close_date
- Answers: "How long was this deal silent before it closed?"

Won deals: Should have small gaps (actively engaged right up to close)
Lost deals: Should have larger gaps (went silent before dying)

This eliminates the 0-day-gap problem by construction - no within-day multiplicity.

Population split:
- won-with-activity (should have recency data)
- lost-with-activity (should have recency data)
- lost-no-activity (report separately - no engagement at all)
- won-no-activity (sanity check - should be rare)

Applies:
1. Bulk event exclusion (750 specific deals from Q016 criteria)
2. Truncation handling (deals with truncated history)
"""
import sys
import json
...
def main():
    ...
    print("=" * 80)
    print("SIGNAL 3 DERIVATION - RECENCY (Last Activity → Close)")
    print("=" * 80)
    print()
    ...
    # Compute LAST-GAP-BEFORE-OUTCOME for each deal
    print("=" * 80)
    print("COMPUTING LAST-GAP-BEFORE-OUTCOME")
    print("=" * 80)
    print()
    ...
    for deal in closed_deals:
        ...
        if days_after_creation > TRUNCATION_THRESHOLD_DAYS:
            stats['truncated_deals'] += 1
            # Still include these deals, but flag for awareness

        # Compute last gap
        last_gap_days = (close_date - last_activity).days

        # Negative gaps indicate close_date before last_activity (data quality issue)
        if last_gap_days < 0:
            continue
    ...
    print(f"  Truncated deals (included): {stats['truncated_deals']}")
    print()

    if stats['won_no_activity'] > 0:
        print(f"⚠️  {stats['won_no_activity']} won deals have no activity (sanity check flag)")
        print()
    ...
    if len(thresholds) > 0:
        ...
        if incorrect_direction > 0:
            ...
            print("   This may indicate insufficient separation or data quality issues")
        else:
            print("✅ All cells show expected direction (lost deals have longer recency)")
    else:
    ...
    output_file = Path(__file__).parent.parent / 'signal3_recency_results.json'
