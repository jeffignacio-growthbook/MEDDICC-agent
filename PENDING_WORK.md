...
### 🟢 LOW: `query_pipeline`'s own synthesis note still says "COVERAGE: 3.38x" inside the quarter-health view (logged 2026-09-25)

- The composed view lifts every primitive's disclosures verbatim, including `query_pipeline._synthesis_note`, which is written for a standalone pipeline answer ("COVERAGE: 3.38x, always against the FY2027 Q3 figure", stage breakdown and top-deal instructions). The composer drops `coverage_ratio` itself and its note tells the model that the weighted figure is the only coverage figure and not to quote the unweighted ratio, but the conflicting text is still in the payload. If a live answer quotes 3.38x, give the composed view a way to carry that note without its standalone formatting rules (tested, not dropped silently).
